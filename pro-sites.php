<?php
/**
 * Plugin Name: WPSAAS Pro
 * Plugin URI:  https://premium.wpmudev.org/project/pro-sites/
 * Description: The ultimate multisite site upgrade plugin, turn regular sites into multiple WPSAAS subscription levels selling access to storage space, premium themes, premium plugins and much more! Author:      WPMU DEV
 * Version:     3.6.1
 * Author:      WPMUDEV
...
class ProSites {
	...
	function __construct() {
		...
		) ); // exclude WPSAAS setting from blog template copies
...
//------------------------------------------------------------------------//
	...
	function init_vars() {
		...
		if ( defined( 'WP_PLUGIN_URL' ) && defined( 'WP_PLUGIN_DIR' ) && file_exists( plugin_dir_path( __FILE__ ) . basename( __FILE__ ) ) ) {
			...
			wp_die( __( 'There was an issue determining where WPSAAS is installed. Please reinstall.', 'psts' ) );
		}
		//Text Domain
	...
	public static function get_default_settings_array() {
		return array(
			...
			'ga_ecommerce'             => 'none',
			'signup_message'           => __( 'Signup for a WPSAAS', 'psts' ),
			'feature_message'          => __( 'Upgrade to LEVEL to access this feature &raquo;', 'psts' ),
			'active_message'           => __( 'Your WPSAAS privileges will expire on: DATE<br />Unless you have canceled your subscription or your site was upgraded via the Bulk Upgrades tool, your WPSAAS privileges will automatically be renewed.', 'psts' ),
			'success_subject'          => __( 'Thank you for becoming a WPSAAS member!', 'psts' ),
			'success_msg'              => __( "Thank you for becoming a WPSAAS member!\n\nWe have received your first subscription payment and you can now access all LEVEL features!\n\nSubscription payments should show on your credit card or bank statement as \"THIS COMPANY\". If you ever need to view, modify, upgrade, or cancel your WPSAAS subscription you can do so here:\nCHECKOUTURL\n\nIf you ever have any billing questions please contact us:\nhttp://mysite.com/contact/\n\nThanks again for joining!", 'psts' ),
			'canceled_subject'         => __( 'Your WPSAAS subscription has been canceled', 'psts' ),
			'canceled_msg'             => __( "Your WPSAAS subscription has been canceled.\n\nYou should continue to have access until ENDDATE.\n\nWe are very sorry to see you go, but we are looking forward to you subscribing to our services again.\n\nYou can resubscribe at any time here:\nCHECKOUTURL\n\nThanks!", 'psts' ),
			'receipt_subject'          => __( 'Your WPSAAS payment receipt', 'psts' ),
			'receipt_msg'              => __( "Your WPSAAS subscription payment was successful!\n\nPAYMENTINFO\n\nSubscription payments should show on your credit card or bank statement as \"YOUR COMPANY\". If you ever need to view, modify, upgrade, or cancel your WPSAAS subscription you can do so here:\nCHECKOUTURL\n\nIf you ever have any billing questions please contact us:\nhttp://mysite.com/contact/\n\nThanks again for being a valued member!", 'psts' ),
			'expired_subject'          => __( 'Your WPSAAS status has expired', 'psts' ),
			'expired_msg'              => __( "Unfortunately the Pro status for your site SITENAME (SITEURL) has lapsed.\n\nYou can renew your WPSAAS status here:\nCHECKOUTURL\n\nIf you're having billing problems please contact us for help:\nhttp://mysite.com/contact/\n\nLooking forward to having you back as a valued member!", 'psts' ),
			'failed_subject'           => __( 'Your WPSAAS subscription payment failed', 'psts' ),
			'failed_msg'               => __( "It seems like there is a problem with your latest WPSAAS subscription payment, sorry about that.\n\nPlease update your payment information or change your payment method as soon as possible to avoid a lapse in WPSAAS features. If you're still having billing problems please contact us for help:\nhttp://mysite.com/contact/\n\nMany thanks again for being a member!", 'psts' ),
			'extension_subject'           => __( 'You have been given free WPSAAS membership.', 'psts' ),
			'extension_msg'               => __( "We have given you WPSAAS access. You will now be able to enjoy all the benefits of being a WPSAAS member.\n\nThese benefits will be available to you until: ENDDATE.\n\nAfter this date your site will revert back to a standard site.\n\nYou can subscribe at any time from the link below:\nCHECKOUTURL\n\nThanks!", 'psts' ),
			'revoked_subject'           => __( 'Your permanent WPSAAS status has changed.', 'psts' ),
			'revoked_msg'               => __( "Your permanent WPSAAS status has been removed. You will continue to have all the benefits of your WPSAAS membership until ENDDATE.\n\nAfter this date your site will revert back to a standard site.\n\nYou can subscribe at any time from the link below:\nCHECKOUTURL\n\nThanks!", 'psts' ),
			'pypl_site'                => 'US',
			'pypl_currency'            => 'USD',
			'pypl_status'              => 'test',
			'pypl_enable_pro'          => 0,
			...
			'bu_checkout_msg'          => __( 'You can upgrade multiple sites at a lower cost by purchasing WPSAAS credits below. After purchasing your credits just come back to this page, search for your sites via the tool at the bottom of the page, and upgrade them to WPSAAS status. Each site is upgraded for one year.', 'psts' ),
			'bu_payment_msg'           => __( 'Depending on your payment method it may take just a few minutes (Credit Card or PayPal funds) or it may take several days (eCheck) for your WPSAAS credits to become available.', 'psts' ),
	...
	function has_setup_fee( $blog_id, $level ) {
		...
		if ( ! self::is_trial( $blog_id ) && is_pro_site( $blog_id ) && ! $this->get_setting( 'apply_setup_fee_upgrade', false ) ) {
			return false;
		} //this is a WPSAAS, not in trial, and admin doesn't want setup fees applied to upgrades
	...
	*/
	...
	function weekly_summary() {
		...
		$text = sprintf( __( '%s active WPSAAS %s since last week', 'psts' ), "<p><span style='font-size: 24px; font-family: arial;'>" . number_format_i18n( $current_total ) . "</span><span style='color: rgb(85, 85, 85);'>", "</span>$active_diff<span style='color: rgb(85, 85, 85);'>" ) . "</span></p>";
	...
	function plug_network_pages() {
		...
		//main page
		$psts_main_page         = add_menu_page( __( 'WPSAAS Pro', 'psts' ), __( 'WPSAAS Pro', 'psts' ), 'manage_network_options', 'psts', array(
			&$this,
			'admin_modify',
		), 'dashicons-plus' );
		...
		//stats page
		$psts_stats_page = add_submenu_page( 'psts', __( 'WPSAAS Statistics', 'psts' ), __( 'Statistics', 'psts' ), 'manage_network_options', 'psts-stats', array(
			&$this,
			'admin_stats',
		) );
		...
		//coupons page
//		$psts_coupons_page_old = add_submenu_page( 'psts', __( 'WPSAAS Coupons', 'psts' ), __( 'Coupons', 'psts' ), 'manage_network_options', 'psts-coupons-old', array(
//			&$this,
//			'admin_coupons'
//		) );
		...
		$psts_gateways_page = add_submenu_page( 'psts', __( 'WPSAAS Gateways', 'psts' ), __( 'Payment Gateways', 'psts' ), 'manage_network_options', 'psts-gateways', array(
		...
		$psts_settings_page = add_submenu_page( 'psts', __( 'WPSAAS Settings', 'psts' ), __( 'Settings', 'psts' ), 'manage_network_options', 'psts-settings', array(
			'ProSites_View_Settings',
			'render_page',
		) );
		...
		//WPSAAS_Setup_Page
		$setup_page = add_submenu_page("psts", "Setup WPSAAS", "Setup WPSAAS", "manage_options", "setup-wpsaas", "setup_wizard", null, 99);

		//checkout page settings
//		$psts_pricing_page_old = add_submenu_page( 'psts', __( 'WPSAAS Pricing Table', 'psts' ), __( 'Pricing Table', 'psts' ), 'manage_network_options', 'psts-pricing-table', array(
//			&$this,
...
//		add_action( 'admin_print_styles-' . $psts_settings_page_old, array( &$this, 'load_settings_style' ) );
		...
		//Add PSTS Style to setup page
		add_action( 'admin_print_styles-' . $setup_page, array( &$this, 'load_setup_style' ) );
...
//		wp_redirect( $this->checkout_url( $blog_id ) );
    ...
    */
	function extend( $blog_id, $extend, $gateway = false, $level = 1, $amount = false, $expires = false, $is_recurring = true, $manual_notify = false, $extend_type = '', $set_trial = false ) {
		...
		if ( $exists != $new_expire ) { //only log if blog expiration date is not changing
			if ( $new_expire >= 9999999999 ) {
				$this->log_action( $blog_id, __( 'WPSAAS status expiration permanently extended.', 'psts' ) );
			} else {
				$this->log_action( $blog_id, sprintf( __( 'WPSAAS status expiration extended until %s.', 'psts' ), date_i18n( get_blog_option( $current_site->blog_id, 'date_format' ), $new_expire ) ) );
			}
		}
		...
		if ( intval( $exists ) <= time() ) { //count reactivating account as upgrade
			...
			if ( $old_level < $level ) {
				$this->log_action( $blog_id, sprintf( __( 'WPSAAS level upgraded from "%s" to "%s".', 'psts' ), $this->get_level_setting( $old_level, 'name' ), $this->get_level_setting( $level, 'name' ) ) );
				do_action( 'psts_upgrade', $blog_id, $level, $old_level );
			} else if ( $old_level > $level ) {
				$this->log_action( $blog_id, sprintf( __( 'WPSAAS level downgraded from "%s" to "%s".', 'psts' ), $this->get_level_setting( $old_level, 'name' ), $this->get_level_setting( $level, 'name' ) ) );
	...
	function withdraw( $blog_id, $withdraw = false, $domain = false ) {
		...
		$this->log_action( $blog_id, __( 'WPSAAS status has been withdrawn.', 'psts' ) );
	...
	*/
	...
	function calc_upgrade( $blog_id, $new_amt, $new_level, $new_period ) {
		...
		if( $new_amt === 0 ) {
			error_log("WPSAAS: Amount can't be zero");
			return false;
		}
	...
	function register_psts_style() {
		...
		// Do not load WPSAAS scripts/styles all over admin.
	...
	*/
	...
	function add_column( $columns ) {
		...
		$columns     = array_merge( $first_array, array( 'psts' => __( 'WPSAAS', 'psts' ) ), $columns );
	...
	function admin_modify() {
		...
		if ( isset( $_POST['psts_transfer_pro'] ) ) {
			...
			if ( ! $new_bid ) {
				...
				echo '<div id="message" class="error"><p>' . __( 'Could not transfer Pro Status: The chosen site already is a WPSAAS. You must remove Pro status and cancel any existing subscriptions tied to that site.', 'psts' ) . '</p></div>';
			} else {
		...
		if ( isset( $_POST['psts_modify'] ) ) {
			...
			if ( isset( $_POST['psts_remove'] ) ) {
				...
				echo '<div id="message" class="updated fade"><p>' . __( 'WPSAAS Status Removed.', 'psts' ) . '</p></div>';
			}
		...
		<h1><?php _e( 'WPSAAS Management', 'psts' ); ?></h1>
		...
		if ( $blog_id ) {
			...
			<div id="poststuff" class="metabox-holder">
				...
				<div style="width: 49%;" class="postbox-container">
					<div class="postbox">
						<h3 class="hndle" style="cursor:auto;">
							<span><?php _e( 'Manually Extend WPSAAS Status', 'psts' ) ?></span></h3>
				...
				<?php if ( is_pro_site( $blog_id ) || has_action( 'psts_modify_form' ) ) { ?>
					<div style="width: 49%;margin-left: 2%;" class="postbox-container">
						<div class="postbox">
							<h3 class="hndle" style="cursor:auto;">
								<span><?php _e( 'Modify WPSAAS Status', 'psts' ) ?></span></h3>
...
if ( $active_pro_sites ) {
	...
	<script type="text/javascript">
		jQuery(document).ready(function ($) {
			//set data
			var pro_sites = {
				label: "<?php echo esc_js(__('Total WPSAAS', 'psts')); ?>",
				color: 3,
...
<div class="wrap">
	...
	<h1><?php _e( 'WPSAAS Statistics', 'psts' ); ?></h1>
...
<?php echo $this->weekly_summary(); ?>
	...
	<div class="metabox-holder">
		...
		<div class="postbox">
			...
			<div class="inside">
				...
				<div style="width:40%;height:300px;float:left;margin-bottom:25px;">
					<h4 style="margin-left:20%;"><?php printf( __( 'Current WPSAAS Levels (%s Total)', 'psts' ), number_format_i18n( $active_pro_sites ) ); ?></h4>
		...
		<div class="postbox">
			<h3 class="hndle" style="cursor:auto;"><span><?php _e( 'WPSAAS History', 'psts' ) ?></span></h3>
		...
		<div class="postbox">
			<h3 class="hndle" style="cursor:auto;"><span><?php _e( 'WPSAAS Term History', 'psts' ) ?></span></h3>
...
<?php
	...
	/**
	 * Checks if a given blog ID is in the WPSAAS table
	 *
	 * @since 3.4.4
	 *
	 * @param int $blog_id
	 ...
	 * @since 3.3.7
	 *
	 * @return settings page for WPSAAS Checkout Page
	 */
	...
	/**
	 * Checks if a particular user role is allowed to perform WPSAAS management
	 *
	 * @param $user_id
	 * @param $roles
	 *
	...
	function activate_user_blog( $domain = false, $trial = true, $period = false, $level = false ) {
		...
		//Login user to follow up the rest of WPSAAS process
		$creds = array(
...
//			$url = sprintf( '<a href="%s">%s</a>', esc_url( $this->checkout_url() ), __( 'Register' ) );
    ...
    * and assigns the WPSAAS level as per the details in site meta
	*
*@param $blog_id
	* @param $user_id
	* @param $password
...
/**
 * Check if a given user is a member of a WPSAAS (at any level)
 *
 * @since 3.0
 *
		...
		<div class="wpsass-setup-wizard-content" data-id="plans-step" style="display:none">
			...
			<div class="wpsass-setup-wizard-row">
				<div class="wpsass-setup-wizard-plan col-4">
					<label><?php _e( 'Currency', 'psts' ) ?></label>
					<select id="psts-currency-select" name="psts[currency]" class="chosen" data-placeholder="<?php echo esc_attr__( 'Enable gateways', 'psts' ); ?>">
						...
						foreach ( $all_currencies as $key => $currency ) {
							...
							<option value="<?php echo $key; ?>"<?php if($settings['currency'] == $key) echo 'selected'; ?>><?php echo esc_attr( strtoupper( $key ) ); ?></option><?php
						} ?>
					</select>
				</div>
				<div class="wpsass-setup-wizard-plan col-4">
		...
		<div class="wpsass-setup-wizard-content" data-id="payment-step" style="display: none;">
			...
			<div class="wpsass-setup-wizard-payment-wrap">
				<div class="wpsass-setup-wizard-payment-block">
					<div class="wpsass-setup-wizard-payment-img">
